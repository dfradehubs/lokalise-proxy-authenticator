...
IMG ?= ghcr.io/dfradehubs/lokalise-proxy-authenticator:latest
...
build: fmt vet check-go-target ## Build CLI binary.
	go build -o bin/lokalise-proxy-authenticator-$(GOOS)-$(GOARCH) cmd/main.go
...
package: check-go-target ## Package binary.
	...
	@if [ "$(OS)" = "linux" ]; then \
		tar --transform="s/lokalise-proxy-authenticator-$(GOOS)-$(GOARCH)/lokalise-proxy-authenticator/" -cvzf dist/$(PACKAGE_NAME) -C bin lokalise-proxy-authenticator-$(GOOS)-$(GOARCH) -C ../ LICENSE README.md; \
	elif [ "$(OS)" = "darwin" ]; then \
		tar -cvzf dist/$(PACKAGE_NAME) -s '/lokalise-proxy-authenticator-$(GOOS)-$(GOARCH)/lokalise-proxy-authenticator/' -C bin lokalise-proxy-authenticator-$(GOOS)-$(GOARCH) -C ../ LICENSE README.md; \
	else \
